class DungeonsGanonsTower extends Dungeons {
  ...
  initNoMajorGlitches() {
    ...
    if (this.buildLocations) {
      let leftside = has("hammer") && canGrapple();
      let rightside = has("firerod") && has("somaria");
  ...
  initMinorGlitches() {
    ...
    this.canEnter.minorGlitches = function () {
      ...
      let ret = crystalCount === 7 && !isBunny(dungeon.subname); // FIXME: Not GT Open Compatible
      let dwdme = new DarkWorldDeathMountainEast("", "", false);
